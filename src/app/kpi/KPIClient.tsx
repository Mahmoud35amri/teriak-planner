...
export default function KPIClient() {
  ...
  // KPI 2 compact: max occupation across all lines × months
  ...
  // KPI 4 compact: min monthly capacity (worst month × line — no annual averaging)
  ...
  // KPI 4 chart: CD per line for selected month
  ...
  const tardinessRows = schedule
    ? schedule.lots
        .map((sl) => ({
          ...
          month: sl.lot.month,
          tardiness: Math.max(0, sl.completionTime - sl.lot.dueDate),
          completionTime: sl.completionTime,
        }))
  ...
  return (
    <AppShell title="Tableau de bord KPI">
      <div className="p-6 space-y-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-xl font-semibold text-gray-900">Tableau de bord KPI</h1>
        ...
        {/* 4 KPI summary cards — always visible */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          {/* KPI 2: Occupation max (from PDP data) */}
          <div className="bg-white border border-gray-200 rounded-xl p-4 text-center">
            <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-2">Occupation max</p>
            {maxOccInfo ? (
              <p
                className="text-4xl font-bold"
                style={{ fontFamily: "var(--font-mono)", color: maxOccInfo.pct > 85 ? "#EF4444" : maxOccInfo.pct > 70 ? "#F59E0B" : "#22C55E" }}
              >
                {maxOccInfo.pct}%
              </p>
            ) : (
          ...
          {/* KPI 4: Capacité disponible (from PDP data) */}
          <div className="bg-white border border-gray-200 rounded-xl p-4 text-center">
            <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-2">Capacité dispo.</p>
            {capaciteInfo ? (
              <p
                className="text-4xl font-bold"
                style={{ fontFamily: "var(--font-mono)", color: capaciteInfo.cd >= 0 ? "#22C55E" : "#EF4444" }}
              >
                {capaciteInfo.cd} h
              </p>
            ) : (
        ...
        {/* KPI 4: Capacité disponible par atelier par mois */}
        {monthlyCH && monthlyTO && (
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            <h2 className="text-sm font-medium text-gray-700 mb-3">
              KPI 4 — Capacité disponible par atelier{" "}
            ...
            <ResponsiveContainer width="100%" height={280}>
              ...
              >
                ...
                />
                <Tooltip
                  formatter={(v) => [`${v} h`, "Capacité disponible"]}
                  labelFormatter={(label) => `Atelier ${label}`}
                />
        ...
        {/* Tardiness table — always visible */}
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          <h2 className="text-sm font-medium text-gray-700 mb-3">
            KPI 3 — Retards par lot{" "}
